...
pub const Stream = struct {
    ...
    pub fn send(
        ...
        if (buffers.len <= QSTACK) {
            buffers = stack_placeholder[0..buffers.len];
        } else {
            buffers = try self.msquic.allocator.alloc(C.Buffer, buffers.len);
            heap_allocated = true;
        }
        ...
        for (data, 0..) |item, i| {
            buffers[i] = C.Buffer{
                .length = @intCast(item.len),
                .buffer = @constCast(item.ptr),
            };
        }
