...
const ServerConnHandler = struct {
    ...
    fn onShutdownInitiatedByTransport(
        ...
        if (status) |err| {
            if (err == msquiczig.MsQuicError.QzConnIdle) {
                atom.?.infoFmt(@src(), "[conn][0x{x}] Successfully shut down on idle.", .{conn_addr}, .{});
            } else {
                atom.?.infoFmt(@src(), "[conn][0x{x}] Shut down by transport, {any}", .{conn_addr, err}, .{});
            }
        } else {
            atom.?.infoFmt(@src(), "[conn][0x{x}] Shut down by transport, error_code: 0x{x}", .{conn_addr, error_code}, .{});
        }
    }
    ...
    fn onShutdownInitiatedByPeer(
        ...
        const conn_addr = @intFromPtr(conn.handle);
        atom.?.infoFmt(@src(), "[conn][0x{x}] Shut down by peer, 0x{x}",
